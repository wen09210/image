		...
		transformSelect: function (opts) {
			...
			method = {
				init: function () {
					...
					if (_this.type == "select-multiple") {
						$ul.find("ul:first").data("trans-element", $ul).addClass("transformMultipleSelect");
					}
				...
				openDrop: function () {
					...
					if (options.addDropdownToBody) {
						childUL.css({
							...
							right: $(window).width() - childLI.offset().left - childLI.outerWidth(),
							margin: 'auto'
						});
					}
				},
                ...
                */
				closeDropDowns: function (e) {
					if (! ($(e.target).closest(".trans-element").length || $(e.target).closest(".transformMultipleSelect").length)) {
						method.closeAllDropdowns();
					}
				},
